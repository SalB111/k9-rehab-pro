  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, marginTop: 2 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 800, color: "#fff", textTransform: "uppercase", letterSpacing: "0.8px" }}>{title}</span>
      </div>
      {children}
    </div>
  );
    ...
    <>
      ...
      <div style={navyCard}>
        ...
        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.7)", marginBottom: 14 }}>
          Document clinical recommendations and owner decisions for each treatment pathway. Complete the relevant block(s) for this patient.
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
        ...
        <StatusBlock icon={FiScissors} title="Surgical" accentColor={C.red}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>Post-operative rehabilitation pathway</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            ...
            {form.surgicalFacility && (
              <span style={{ fontSize: 10, fontWeight: 500, padding: "3px 8px", borderRadius: 4, background: C.bg, color: C.text }}>
                Facility: {form.surgicalFacility}
              </span>
            )}
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Clinical Recommendation</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              ...
              placeholder="e.g. TPLO recommended for complete CCL rupture, good surgical candidate" />
          </div>
          <div>
            <label style={S.label}>Owner Decision</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.surgicalOwnerDecision || ""} onChange={e => setField("surgicalOwnerDecision", e.target.value)}
              placeholder="e.g. Owner elected surgery, scheduled for next week" />
          </div>
        </StatusBlock>

        {/* ── BLOCK 2: NON-SURGICAL / CONSERVATIVE ── */}
        <StatusBlock icon={FiShield} title="Non-Surgical / Conservative" accentColor={C.teal}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>Conservative management without surgical intervention</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            {form.diagnosis && (
          ...
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Conservative Approach Rationale</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.conservativeRationale || ""} onChange={e => setField("conservativeRationale", e.target.value)}
              placeholder="e.g. Partial CCL tear, young patient, good muscle mass — conservative trial 8 weeks before surgical re-evaluation" />
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Clinical Recommendation</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.conservativeClinicalRec || ""} onChange={e => setField("conservativeClinicalRec", e.target.value)}
              placeholder="e.g. Structured rehab program with weight management, modality support, and re-evaluation at 4 weeks" />
          </div>
          <div>
            <label style={S.label}>Owner Decision</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.conservativeOwnerDecision || ""} onChange={e => setField("conservativeOwnerDecision", e.target.value)}
              placeholder="e.g. Owner prefers conservative approach, committed to rehab schedule" />
          </div>
        </StatusBlock>

        {/* ── BLOCK 3: POST-SURGICAL RECOVERY ── */}
        <StatusBlock icon={FiActivity} title="Post-Surgical Recovery" accentColor={C.green}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>Current recovery status and milestones</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            {form.postOpDay && (
          ...
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Current Recovery Status</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.recoveryStatus || ""} onChange={e => setField("recoveryStatus", e.target.value)}
              placeholder="e.g. Progressing well, toe-touching weight bearing, mild swelling resolved, starting active ROM" />
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Recovery Milestones</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 44, resize: "vertical", fontFamily: "inherit" }}
              ...
              placeholder="e.g. Sutures removed POD 14, cleared for controlled leash walks POD 21" />
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Clinical Recommendation</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.postSurgRecoveryClinicalRec || ""} onChange={e => setField("postSurgRecoveryClinicalRec", e.target.value)}
              placeholder="e.g. Progress to Phase 2 exercises, begin underwater treadmill, increase walk duration" />
          </div>
          <div>
            <label style={S.label}>Owner Decision</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 44, resize: "vertical", fontFamily: "inherit" }}
              value={form.postSurgRecoveryOwnerDecision || ""} onChange={e => setField("postSurgRecoveryOwnerDecision", e.target.value)}
              placeholder="e.g. Owner agrees to increase frequency to 2x/week, reports improved comfort at home" />
          </div>
        </StatusBlock>

        {/* ── BLOCK 4: PALLIATIVE / COMFORT CARE ── */}
        <StatusBlock icon={FiHeart} title="Palliative / Comfort Care" accentColor={C.purple}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>Quality of life focused — comfort and function preservation</div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Comfort Priorities</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.palliativeComfortPriorities || ""} onChange={e => setField("palliativeComfortPriorities", e.target.value)}
              placeholder="e.g. Pain management, mobility preservation, appetite maintenance, owner bonding time" />
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Quality of Life Assessment</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.palliativeQoL || ""} onChange={e => setField("palliativeQoL", e.target.value)}
              placeholder="e.g. HHHHHMM Scale: Hurt 7, Hunger 8, Hydration 9, Hygiene 7, Happiness 6, Mobility 4, More Good Days 6 = 47/70" />
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={S.label}>Clinical Recommendation</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 48, resize: "vertical", fontFamily: "inherit" }}
              value={form.palliativeClinicalRec || ""} onChange={e => setField("palliativeClinicalRec", e.target.value)}
              placeholder="e.g. Gentle passive ROM, heat therapy, massage, controlled short walks — focus on comfort not progression" />
          </div>
          <div>
            <label style={S.label}>Owner Decision</label>
            <textarea style={{ ...S.input, border: `1.5px solid ${C.border}`, minHeight: 44, resize: "vertical", fontFamily: "inherit" }}
              value={form.palliativeOwnerDecision || ""} onChange={e => setField("palliativeOwnerDecision", e.target.value)}
              placeholder="e.g. Owner understands prognosis, focused on quality time, agrees to comfort-focused protocol" />
          </div>
        </StatusBlock>
      </div>
      ...
      <div style={navyCard}>
        ...
        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.7)", marginBottom: 10 }}>
          Expanded notes for multi-pathway patients, interdisciplinary communication, or complex cases
        </div>
        <textarea
          ...
          placeholder="e.g. Patient presenting for rehab after owner initially declined surgery 6 weeks ago. Conservative management partially effective — owner now reconsidering surgical option if rehab plateau reached at 4-week re-eval."
        />
      </div>
